    ...
    public class AccountController : Controller
    {
       ...
       //"User Id= hr;Password=hr";
        ...
        public IActionResult Login(string username, string password)
        {
            ...
            var objuser = _context.Cemps.Where(b => b.Cempid == username&&b.status!="1").FirstOrDefault();
            if(objuser==null)
            {
            ...
            MailExpire mail = new MailExpire();
            if (mail.CH < DateTime.Now.Date)
            {
                ViewBag.error = "...  ";
                loggerx.Error("...  " + username);
                return View("Show");
            }
                ...
                var claims = new List<Claim>
            {
                ...
                try
                {
                    ...
                    objuser.CEMPBIRTHDATEHIJRA= Convert.ToDateTime(datebirth);
                    // check approval for eval details 
                    var reqevalfor = _context.EvalDetailss2.Where(h => h.OfferedRequestFrom == HttpContext.Session.GetString("username")).OrderBy(x=>x.OfferedDetailsSerial).LastOrDefault();
                    if (reqevalfor != null)
                    {
                        // 1-02-2023
                        var reqevalfor01 = _context.EvalDetailss.Where(h => h.OfferedRequestFrom == HttpContext.Session.GetString("username")).OrderBy(x => x.OfferedDetailsSerial).LastOrDefault();
                        if (reqevalfor01 != null) {
                            reqevalfor01.OfferedRequestTo = MANAGERID;
                            reqevalfor01.OfferedRequestTo2 = MANAGERID2;
                            _context.Update(reqevalfor01);
                        }
                        //end 1-02-2023
                    ...
                    // check approval for TasksManagement 

                    var reqevalfor0 = _context.TMDetailss2.Where(h => h.CourcesIdoffered == Convert.ToInt32(HttpContext.Session.GetString("empdepid"))).FirstOrDefault();
                    if (reqevalfor0 != null)
                    {
                        reqevalfor0.OfferedRequestTo = MANAGERID;
                        //reqevalfor0.OfferedRequestTo2 = MANAGERID2;

                        _context.Update(reqevalfor0);
                    }

                    else
                    {
                        loggerx.Error("ليس له طلب خدمة اعتماد أدارة   " + username);
                    }
                ...
                {
                    loggerx.Error("لم يتم حفظ بيانات الموظف لكي يظهر في معلوماتي لخطء ما في البيانات   " + username);
                }
                finally
                {
                ...
                //TMDtails
                List<TMDetail2> TMDetails2 = _context.TMDetailss2.ToList();

                List<TMRequestTypeId2> TMRequestTypeId2 = _context.TMRequestTypeIds2.ToList();
                ...
                var xx11 = from e in TMRequestTypeId2
                           join m in TMDetails2 on e.CourcesIdoffered equals m.CourcesIdoffered into table77
                          from m in table77.ToList().Distinct()
                          where (m.OfferedRequestTo == HttpContext.Session.GetString("empid") && m.OfferedRequestTo3 == "0") || (m.OfferedRequestTo2 == HttpContext.Session.GetString("empid") && m.OfferedRequestTo4 == "0") || (m.Offeredoption == HttpContext.Session.GetString("empid") && m.OfferedRequestTo5 == "0") && e.OfferedRequestType == 0
                          select (e.CourcesIdoffered).ToString();
                TempData["OfferedRequestTypeId11"] = xx11.ToList().Count();


                TempData["total"] = yy.ToList().Count() + xx.ToList().Count() + xx2.ToList().Count() + xx3.ToList().Count()+ xx4.ToList().Count()+ xx5.ToList().Count()+xx6.ToList().Count()+xx7.ToList().Count()+ xx8.ToList().Count()+ xx9.ToList().Count()+ xx10.ToList().Count()+ xx11.ToList().Count();
